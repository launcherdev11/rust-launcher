...
fn preload_system_wayland_client() {
    ...
    let Some(wayland) = resolve_system_wayland_client() else {
        return;
    };

    let mut parts: Vec<OsString> = Vec::new();
    parts.push(wayland.into_os_string());

    if let Ok(existing) = env::var("LD_PRELOAD") {
        for segment in existing.split(':').filter(|s| !s.is_empty()) {
            let already = parts.iter().any(|p| p == segment);
            if !already {
                parts.push(OsString::from(segment));
            }
        }
    }
